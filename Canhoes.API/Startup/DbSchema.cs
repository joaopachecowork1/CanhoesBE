...
BEGIN
  ...
  -- Drop ContentBytes column if it exists (migrated to filesystem storage)
  IF COL_LENGTH('HubPostMedia', 'ContentBytes') IS NOT NULL
    ALTER TABLE HubPostMedia DROP COLUMN ContentBytes;
END
");
        }
        catch
